...
def compute_homography_warp(points2D,image,imagename):
    ...
    im_dst = cv2.warpPerspective(image, M, (224,224))
...
def predict(inputs,model,checkpoint_dir,with_seg, method):
    ...
    image_batch = tf.placeholder(shape=[None, 224, 224, 1], dtype=tf.float32)
    ...
    def drawlandmark(image,points2D,outname,visibility):
        ...
        for i in range(points2D.shape[1]):
            if visibility[i]==1:
                cv2.circle(image_landmark,(int(np.round(points2D[0,i])),int(np.round(points2D[1,i]))), 2, colors[i], -1)
            else:
...
predict("IR","single","/home/z003xr2y/data/Multi-task_CNN/checkpoints_IR_landmark_single_domain_trans/",False,"checkpoints_IR_landmark_single_domain_trans")
