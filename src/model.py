...
def disp_net_coord(tgt_image, is_training=True):
    ...
    with tf.variable_scope('pose_net',reuse = tf.AUTO_REUSE) as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
            ...
            fc1 = tf.layers.dense(inputs=cnv_flat, units=max_features*2, activation=tf.nn.leaky_relu)
            coord_final = tf.layers.dense(inputs=fc1, units=8, activation=None)

            return cnv3b[:,:,:,0:28],coord_final



def coord_conv_net(tgt_image, is_training=True):

    batch_norm_params = {'is_training': is_training,'decay':0.9}
    H = tgt_image.get_shape()[1].value
    W = tgt_image.get_shape()[2].value
    max_features=512
    with tf.variable_scope('coconv_net',reuse = tf.AUTO_REUSE) as sc:
        end_points_collection = sc.original_name_scope + '_end_points'
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose],
                            normalizer_fn=slim.batch_norm,
                            normalizer_params=batch_norm_params,
                            ...
                            outputs_collections=end_points_collection):
            tgt_image = CoordinateChannel2D()(tgt_image)
            cnv1  = slim.conv2d(tgt_image[:,:,:,:], 32,  [1, 1], stride=1, scope='cnv1')
            cnv2  = slim.conv2d(tgt_image[:,:,:,:], 32,  [1, 1], stride=1, scope='cnv2')
            cnv3  = slim.conv2d(tgt_image[:,:,:,:], 32,  [1, 1], stride=1, scope='cnv3')
            cnv4  = slim.conv2d(tgt_image[:,:,:,:], 32,  [3, 3], stride=1, scope='cnv4')
            cnv5  = slim.conv2d(tgt_image[:,:,:,:], 8,  [3, 3], stride=1, scope='cnv5')
            output = tf.layers.average_pooling2d(cnv5, [H, W],[H,W])

            return cnv5[:,:,:,0:28],output
